...
parser.add_argument("-T",dest= "target_charge", required = True, type = int)
arg = parser.parse_args()
pdbfile = arg.prot
center_coordinates = arg.center
center_coordinates = center_coordinates.strip('[')
center_coordinates = center_coordinates.strip(']')
center_coordinates = center_coordinates.split(':')
sphere_radius = arg.radius
target_charge = arg.target_charge
...
"""def salt_bridge(coord1,coord2,distance):
...
with open(pdbfile, "r") as inp:
    for s in inp:
        if s[17:20]+s[23:26] in salt_bridge:
            ...
            res_salt_bridge.append("ASP101")
inp.close()
res_salt_bridge = list(dict.fromkeys(res_salt_bridge))

##center residue coordinates
#with open(pdbfile, "r") as input_pdb:
#    center_coord = {}
#    center_coordinates = []
#    center = center_residue[0]
#    for elem in input_pdb:
#        if not elem[17:20] == ("SOL") or "POP":
#            if center_residue[1] == elem[21:22]:
#                if elem[23:26] == (center):
#                    if elem[13:15] == "CA":
#                        center_coord[elem[17:20]+center] =([float(elem[32:38]),
#                                                           float(elem[40:46]),
#                                                           float(elem[47:54])
#                                                          ])
#                elif elem[23:26] == (" "+center):
#                    if elem[13:15] == "CA":
#                        center_coord[elem[17:20]+center] = ([float(elem[32:38]),
#                                                            float(elem[40:46]),
#                                                            float(elem[47:54])
...
#                                                            float(elem[47:54])
#                                                           ])
#for key,value in center_coord.items():
#    temp = [key,value]
#    center_coordinates.append(temp)
##print(center_coordinates[0][1][2])
...
with open("neg_aa.txt", "r") as charged_neg:
    ...
    for charged2 in charged_neg:
        if charged2[0:3] != "TER":
            ...
            if charged2[17:20] == "ASP" or "GLU":
                if "CA" == charged2[13:15]:
                    if distance < (sphere_radius):
...
"""
for element in residues_to_decharge_neg:
    if n < 0:
        if element in too_close:
          #  print(element + " will not be decharged")
            residues_to_decharge_neg.remove(element)

for element in residues_to_decharge_pos:
    if n > 0:
        if element in too_close:
         #   print(element + " will not be decharged")
            residues_to_decharge_pos.remove(element)"""


#write new pdb file for decharging of pos aa
def write_pdb(list1):
    with open(pdbfile, "r") as input_pdb:
...
#define which residues will be decharged
if target_charge > 0:
    list1=residues_to_decharge_neg
elif target_charge < 0:
    list1 = residues_to_decharge_pos

print("decharging residues:")
print(list1)
