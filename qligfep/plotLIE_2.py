...
#        LIEs     = {}
#        self.MDs  = 0
#        self.Reps = glob.glob(self.LIEdir + '/FEP1/*/[1-9]') + glob.glob(self.LIEdir + '/FEP1/*/10')
#        for rep in self.Reps:
#            LIEs[rep] = sorted(glob.glob(rep + '/md_LIE_0[1-9].log') + glob.glob(rep + '/md_LIE_10.log'))
...
#        self.el  = [x for x in self.el if x != []]
    ...
    def calc_LIE(self):
        ...
        self.plot_vdw = copy.deepcopy(data.vdw)
        self.plot_el  = copy.deepcopy(data.el)
        ...
        for rep in self.plot_vdw:
            if len(rep) < (self.MDs*10001):
                x = (self.MDs*10001) - len(rep)
                for i in range(x):
                    rep.append(rep[-1])
        for rep in self.plot_el:
            if len(rep) < (self.MDs*10001):
                x = (self.MDs*10001) - len(rep)
                for i in range(x):
                    rep.append(rep[-1])
        ...
        self.plot_vdw = np.array(self.plot_vdw)
        self.plot_el  = np.array(self.plot_el)
        print(self.plot_vdw)
        
        self.x_axis = range(0, len(self.plot_vdw[0]))
        
        self.avg_vdw = np.nanmean(self.vdw, axis=0)
        self.avg_el  = np.nanmean(self.el, axis=0)
        
        self.avg_plot_vdw = np.nanmean(self.plot_vdw, axis=0)
        self.avg_plot_el  = np.nanmean(self.plot_el, axis=0)
        
        self.avg_fl_vdw = self.fl_en_avg(self.avg_plot_vdw)
        self.avg_fl_el = self.fl_en_avg(self.avg_plot_el)
